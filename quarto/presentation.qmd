...
::: {.notes}
Dans un premier temps on choisit les variables qui serviront dans nos modèles et on choisit les modèles que l'on va utiliser. On a décider de se concentrer sur 6 modèles pour ne pas être trop exhaustif (RF, Boosting, Ridge, MLP, KNN, SVM). 

Une fois les modèles et les variables choisis on passe à l'optimisation des paramétres par le biais d'une CV. Pour chaque modèle on a decidé d'optimiser entre 2 et 3 paramètres.

Une fois la CV terminé tous les résultats sont stocké dans un CSV avec les score, ET, et valeur des hyperparamètres.
:::
...
Dès que l'on a les meilleurs hyperparamètres pour chaqu'un des modèles on peut désormais passer à la prédiction sur les 800 vins que l'on a laissé de coter pour tester nos modèles.

Une fois encore on stock tous les résultats dans un CSV, pour chaque vin on a la valeur réel et la prédiction pour chaqu'un des modèles.
...
La différence entre les deux premières méthodes c'est que l'une va tourner en local alors que l'autre est sur le cloud streamlit. Le déploiement sur le cloud est assez simple : on link le repo Github, le pyproject.toml et poetry.lock qui installe les dépendances et c'est parti 🚀.
:::
...
6. **RUN addgroup --system app && adduser --system --group app** $\Rightarrow$ Crée un groupe système appelé app et un utilisateur système également appelé app.
...
## 🏗️ Le conteneur en action ! {.smaller}
